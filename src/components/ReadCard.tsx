    ...
    const fetchStudentData = async (registerNumber: string) => {
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                setEventLog((prev) => [
                    ...prev,
                    `💰 Balance: Rs.${result.data.balance.toFixed(2)}`,
                ]);
            } else {
                setEventLog((prev) => [
                    ...prev,
    ...
    const processTransaction = async () => {
        ...
        try {
            ...
            if (studentData.balance < totalAmount) {
                setEventLog((prev) => [
                    ...prev,
                    `❌ Insufficient balance. Need: Rs.${totalAmount.toFixed(2)}, Available: Rs.${studentData.balance.toFixed(2)}`,
                ]);
            ...
            if (result.success) {
                ...
                setEventLog((prev) => [
                    ...prev,
                    `💰 New balance: Rs.${(studentData.balance - totalAmount).toFixed(2)}`,
                ]);
                    ...
                    <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                            ...
                            <div>
                                ...
                                >
                                    Rs.{studentData.balance.toFixed(2)}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                {canProcessTransaction && (
                    <div className="flex flex-col gap-2">
                        {hasInsufficientFunds && (
                            <div className="bg-red-50 p-3 rounded border border-red-200">
                                <p className="text-red-700 text-sm">
                                    ...
                                    {(
                                        calculateTotal() - studentData.balance
                                    ).toFixed(2)}{" "}
                                    more.
                                </p>
                            </div>
                        ...
                        >
                            {isProcessing
                                ? "Processing..."
                                : `Pay Rs.${calculateTotal().toFixed(2)}`}
                        </Button>
                    </div>
                )}

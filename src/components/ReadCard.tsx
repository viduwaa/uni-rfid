  ...
  (
    ...
    useEffect(() => {
      ...
      socketConnection.on("nfc-reader-status", (status) => {
        ...
        if (status.status === "connected") {
          ...
          setEventLog((prev) => [...prev, `❌ Reader error: ${status.error}`]);
        }
      });

      socketConnection.on("nfc-swipe", async (data) => {
        console.log("💳 Card swiped:", data);
        setEventLog((prev) => [...prev, `💳 Card detected: ${data.uid}`]);
        ...
        if (data.data && data.data.register_number) {
          ...
          setEventLog((prev) => [...prev, "❌ No valid data found on card"]);
        }
      });

      socketConnection.on("nfc-swipe-end", (data) => {
        console.log("👋 Card removed:", data);
        setEventLog((prev) => [...prev, `👋 Card removed: ${data.uid}`]);
    ...
    const fetchStudentData = async (registerNumber: string) => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setEventLog((prev) => [
            ...prev,
            `💰 Balance: ${formatCurrency(result.data.balance)}`,
          ]);

          // Auto-process payment if enabled and cart has items
          if (autoProcessPayment && cart.length > 0) {
            setEventLog((prev) => [...prev, `🚀 Auto-processing payment...`]);
    ...
    const processTransaction = async () => {
      ...
      try {
        ...
        if (studentData.balance < totalAmount) {
          setEventLog((prev) => [
            ...prev,
            `❌ Insufficient balance. Need: ${formatCurrency(totalAmount)}, Available: ${formatCurrency(studentData.balance)}`,
          ]);
        ...
        if (result.success) {
          ...
          setEventLog((prev) => [
            ...prev,
            `💰 New balance: ${formatCurrency(studentData.balance - totalAmount)}`,
          ]);

          // Update local student data
          setStudentData((prev) =>
            prev ? { ...prev, balance: prev.balance - totalAmount } : null
          );
    ...
    const processManualPayment = async (manualAmount?: number) => {
      ...
      try {
        ...
        setEventLog((prev) => [
          ...prev,
          `🔍 Sending manual payment: ${formatCurrency(manualAmount || totalAmount)}`,
        ]);
    ...
    const hasInsufficientFunds =
      studentData && cart.length > 0 && studentData.balance < calculateTotal();
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                ...
                size="sm"
              >
                {waitingForCard ? "Waiting for Card..." : "Start Reading"}
              </Button>
          ...
          {studentData && (
            <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                <div>
                  <strong>Name:</strong> {studentData.full_name}
                </div>
                <div>
                  <strong>Register No:</strong> {studentData.register_number}
                </div>
                <div>
              ...
              {hasInsufficientFunds && (
                <div className="bg-red-50 p-3 rounded border border-red-200">
                  <p className="text-red-700 text-sm">
                    ...
                    {formatCurrency(calculateTotal() - studentData.balance)}{" "}
                    more.
                  </p>
                </div>
              )}

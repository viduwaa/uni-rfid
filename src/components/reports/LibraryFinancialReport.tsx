...
export default function LibraryFinancialReport() {
  ...
  const [summary, setSummary] = useState<LibraryFinancialSummary | null>(null);
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <ReportSummaryCard
            ...
            value={formatCurrency(summary.total_fines_generated)}
            icon={<DollarSign className="h-4 w-4 text-red-600" />}
          />
          <ReportSummaryCard
            title="Fines Collected"
            value={formatCurrency(summary.total_fines_collected)}
            icon={<CheckCircle className="h-4 w-4 text-green-600" />}
          />
          <ReportSummaryCard
            title="Pending Fines"
            value={formatCurrency(summary.pending_fines)}
            icon={<AlertCircle className="h-4 w-4 text-orange-600" />}
          />
          <ReportSummaryCard
            title="Waived Fines"
            value={formatCurrency(summary.waived_fines)}

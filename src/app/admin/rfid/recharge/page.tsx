...
export default function RechargeCards() {
    ...
    try {
      ...
      if (foundCard) {
        ...
        toast.success("Card recognized!", {
          description: `${foundCard.full_name} - ${formatCurrency(foundCard.balance)}`,
        });
        setEventLog((prev) => [
          ...prev,
        ...
        const response = await fetch(`/api/rfid/cards?card_uid=${cardUID}`);
        const data = await response.json();
        ...
        if (data.success && data.data && data.data.length > 0) {
          ...
          if (card.status === "ACTIVE") {
            ...
            toast.success("Card recognized!", {
              description: `${card.full_name} - ${formatCurrency(card.balance)}`,
            });
            setEventLog((prev) => [
              ...prev,
            ...
            toast.error(`Card is ${card.status} and cannot be recharged`);
            setEventLog((prev) => [...prev, `❌ Card is ${card.status}`]);
          }
        } else {
  ...
  const validateAmount = (amount: string): boolean => {
    ...
    if (numAmount > 10000) {
      setValidationError(`Amount cannot exceed ${formatCurrency(10000)}`);
      return false;
    }
  ...
  const handleRecharge = async () => {
    ...
    try {
      ...
      toast.success("Card recharged successfully!", {
        description: `Added ${formatCurrency(amount)} to ${selectedCard.full_name}'s card`,
      });
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant={
                  nfcStatus.status === "connected" ? "default" : "secondary"
                }
                className="flex items-center gap-1"
              >
                <Radio
                  className={`h-3 w-3 ${
                    nfcStatus.status === "connected" ? "animate-pulse" : ""
                  }`}
                />
                {nfcStatus.status === "connected" ? "NFC Ready" : "NFC Offline"}
              </Badge>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button
              ...
              disabled={nfcStatus.status !== "connected" || waitingForCard}
              variant="outline"
                ...
                filteredCards.map((card) => (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-all hover:shadow-md ${
                      selectedCard?.card_uid === card.card_uid
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        ...
                        <Badge
                          variant="secondary"
                          className={getFacultyBadgeColor(card.faculty)}
                        >
            ...
            {selectedCard ? (
              <>
                ...
                <div className="p-4 bg-emerald-50 dark:bg-emerald-950 rounded-lg border border-emerald-200">
                  ...
                  <p className="font-bold text-lg">{selectedCard.full_name}</p>
                  <p className="text-sm text-muted-foreground">
                ...
                <Button
                  ...
                  disabled={
                    !rechargeAmount || !!validationError || isRecharging
                  }
                  className="w-full"
                  size="lg"
                >
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-lg font-medium">No Card Selected</p>
                <p className="text-sm">
                  Please select a card from the list to continue
                </p>
              </div>
            )}
          </CardContent>

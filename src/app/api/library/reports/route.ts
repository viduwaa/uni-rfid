        ...
        switch (reportType) {
            ...
            case "circulation":
                if (!startDate || !endDate) {
                    return NextResponse.json(
                        {
                            error: "Start date and end date are required for circulation report",
                        },
                        { status: 400 }
                    );
                }
            ...
            case "financial":
                if (!startDate || !endDate) {
                    return NextResponse.json(
                        {
                            error: "Start date and end date are required for financial report",
                        },
                        { status: 400 }
                    );
                }
            ...
            default:
                return NextResponse.json(
                    {
                        error: "Invalid report type. Use: stats, circulation, financial, or inventory",
                    },
                    { status: 400 }
                );
        }

...
export default function BookReturns() {
  ...
  useEffect(() => {
    const fetchStudentLoans = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error("Failed to fetch student loans");
  ...
  const handleStudentVerified = async (student: any) => {
    ...
    if (verifiedStudent.pending_fines > 0) {
      try {
        const finePaymentResponse = await fetch("/api/library/fines/bulk-pay", {
          method: "POST",
        ...
        toast.warning("Could not auto-pay pending fines. Please pay manually.");
      }
    }
  ...
  const removeFromReturns = (loanId: string) => {
    setSelectedReturns(selectedReturns.filter((item) => item.id !== loanId));
  };
    ...
    try {
      ...
      toast.success(
        `Successfully returned ${selectedReturns.length} books` +
          (totalFines > 0 ? ` (Fine: ${formatCurrency(totalFines)})` : "")
      );
        ...
        if (studentResponse.ok) {
          ...
          if (studentData.success) {
            ...
            const updatedStudent = {
              ...
              checkout_restrictions: studentData.data.checkout_restrictions,
            };
  ...
  return (
    <div className="container mx-auto py-6 px-4 space-y-6">
      <PageHeader
        ...
        centerIcon={<BookX className="h-8 w-8 text-primary mx-auto" />}
        right={
            ...
            >
              ...
              <TabsContent value="student" className="space-y-4">
                <div className="text-sm text-muted-foreground mb-3">
                  Scan student's RFID card to view all their current loans
                </div>
                <RFIDStudentReader onStudentVerified={handleStudentVerified} />
              </TabsContent>

              <TabsContent value="barcode" className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="barcodeSearch">Scan Book Barcode</Label>
                  ...
                  <div className="text-sm text-muted-foreground">
                    Scan book barcode to directly add to returns
                  </div>
                </div>
              </TabsContent>
            </Tabs>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {selectedStudent ? "Current Loans" : "Ready to Return"} (
          ...
          <CardContent className="space-y-4">
            ...
            {selectedStudent && studentLoans.length > 0 && (
              <div className="space-y-2 max-h-64 overflow-y-auto">
                <h4 className="font-medium text-sm">Available for Return:</h4>
            ...
            {selectedReturns.length > 0 && (
              <>
                ...
                <div className="space-y-2">
                  <h4 className="font-medium text-sm">Ready to Return:</h4>
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-muted-foreground mt-1">
                            ...
                            {item.is_late && (
                              <span className="text-red-600 ml-2">
                                ({item.days_overdue} days late - Fine:{" "}
            ...
            {!selectedStudent && selectedReturns.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>Scan student card or book barcode to start returns</p>
              </div>
            )}
          </CardContent>
        </Card>
            ...
            <div className="space-y-4">
              {totalFines > 0 && (
                <div className="p-4 border border-yellow-200 bg-yellow-50 dark:bg-yellow-950 dark:border-yellow-800 rounded-lg">
                  ...
                  <div className="text-sm text-yellow-700 dark:text-yellow-300">
                    Total books: {selectedReturns.length} | Late returns:{" "}
                    {selectedReturns.filter((item) => item.is_late).length} |
                    Total fines: {formatCurrency(totalFines)}
                  </div>
                </div>
              )}
              ...
              >
                {returnLoading ? (
                  ...
                  <>
                    Process Return of {selectedReturns.length} Books
                    {totalFines > 0 && ` (Fine: ${formatCurrency(totalFines)})`}
                  </>
                )}
              </Button>

  ...
  try {
    ...
    const bookQuery = `
      INSERT INTO books (isbn, title, author, publisher, publication_year, category, 
                        description, location, total_copies, available_copies, created_by)
...
export async function checkoutBook(
  ...
  try {
    ...
    // Create loan record
    const loanQuery = `
      INSERT INTO book_loans (student_id, book_copy_id, due_date, notes, issued_by, status)
...
export async function updateMemberStatus(
  ...
  const query = `
    ...
    RETURNING *
  `;
  const result = await pool.query(query, [studentId, status, maxBooks, notes]);
  return result.rows[0];
}
...
export async function getInventoryReport(): Promise<any> {
  ...
  return {
    ...
    checked_out_copies: parseInt(totalResult.rows[0].checked_out_copies) || 0,
...
export async function getPopularBooksReport(
  ...
  const result = await pool.query(
    ...
    WHERE bl.loan_date >= $1 AND bl.loan_date <= $2
